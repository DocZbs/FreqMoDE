...
class CondRouterMLP(nn.Module):
    ...
    def forward(self, x):
        # Normalize input to prevent extreme activations
        x = x / (x.std(dim=-1, keepdim=True) + 1e-6)
        output = self.mlp(x)
        # Clamp output to prevent NaN in downstream softmax
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Linear):
                # Use smaller std for stability (Xavier-like initialization)
...
class RouterCond(nn.Module):
    ...
    def _compute_probabilities(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        # Replace any NaNs or infs in logits for stability.
    ...
    def _select_experts_without_shared(self, probs: torch.Tensor, input_shape: torch.Size):
        ...
        # Handle NaN/inf probabilities by replacing with uniform distribution
        if not torch.isfinite(flat_probs).all():
            logging.warning("Router probabilities contain NaN/inf, using topk selection")
            top_k_indices = flat_probs.nan_to_num(0.0).topk(self.top_k, dim=-1).indices
        elif self.training and not self.use_argmax:
...
class NoiseBlockMoE(nn.Module):
    ...
    def forward(self, x, c, context=None, custom_attn_mask=None):
        ...
        if not self.training and self.fused_experts:  # Only use cache if it's not empty
            noise_key = c.mean().item()
            fused_mlp = self.fused_experts.get(noise_key)
            if fused_mlp is not None:
                batch_tokens = x.size(0) * x.size(1)
                self.total_tokens_processed += batch_tokens

                routing = self.routing_info.get(noise_key)
                if routing is not None:
                    indices = routing.get("indices")
                    probs = routing.get("probs")
                    if indices is not None and probs is not None:
                        try:
                            for expert_idx, expert_prob in zip(indices, probs):
                                expert_idx = int(expert_idx)
                                if 0 <= expert_idx < self.inference_expert_usage.numel():
                                    self.inference_expert_usage[expert_idx] += float(expert_prob) * batch_tokens
